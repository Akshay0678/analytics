...
p2sales = predict(fit2, newdata=ndata2)   #By default, type = 'resposnse'
cbind(ndata2, p2sales)

#Assumptions
par(mfrow=c(2,2))  ##mfrow = multiple frames per row, 2x2
plot(fit2)
par(mfrow=c(1,1))
...
#create model with t
#commenting below lines, cz removing so many outliers is decreasing the correlation
#fit4 = lm(sales ~ price + promotion, data=omni[-c(1,16,27),])
#plot(fit4,4)
#summary(fit4)
#mean(residuals(fit4))
...
plot(fit3,which=1)  # no pattern, equal variance
plot(fit3,2)  # Residuals are normally distributed
plot(fit3,3)  # No hetero-scedascity
plot(fit3,4)  # tells outliers which affect model


#divided dataset into parts
#training set - 70%
#testing set - 30%
